import { runCommand, runCommandSync } from './exec.js';
export async function getColimaStatus() {
...
export async function getColimaVMStats() {
    ...
    const diskResult = await runCommand(`colima ssh -- df -B1 / 2>/dev/null | tail -1 | awk '{print $2, $3}'`);
    ...
    // Get VM memory usage
    const memResult = await runCommand(`colima ssh -- free -b 2>/dev/null | grep Mem | awk '{print $2, $3}'`);
    ...
    // Get CPU usage
    const cpuResult = await runCommand(`colima ssh -- top -bn1 2>/dev/null | grep "Cpu(s)" | awk '{print $2}'`);
    let cpuPercent = 0;
...
export function getColimaStatusSync() {
    try {
 ...
 */
export async function getColimaDataDisk() {
    const status = await getColimaStatus();
    if (!status || !status.running) {
        return null;
    }
    ...
    const diskResult = await runCommand(`colima ssh -- df -B1 /mnt/lima-colima 2>/dev/null || colima ssh -- df -B1 / 2>/dev/null | tail -1`);
    if (!diskResult.success || !diskResult.stdout) {
        return null;
    }
