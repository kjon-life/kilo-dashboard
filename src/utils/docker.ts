import { runCommand, parseBytes, formatBytes } from './exec.js';
...
export interface DockerSystemDF {
  ...
  containers: { total: number; active: number; size: number; reclaimable: number };
  volumes: { total: number; active: number; size: number; reclaimable: number };
  buildCache: { total: number; active: number; size: number; reclaimable: number };
}
...
export async function getContainerStats(): Promise<Map<string, Partial<DockerContainer>>> {
  const format = '{{.ID}}|{{.CPUPerc}}|{{.MemUsage}}|{{.MemPerc}}|{{.NetIO}}|{{.BlockIO}}';
  const result = await runCommand(`docker stats --no-stream --format "${format}"`);
  
  const stats = new Map<string, Partial<DockerContainer>>();
...
export async function getVolumes(): Promise<DockerVolume[]> {
  ...
  const containerVolumes = await runCommand(
    `docker ps -a --format '{{.Mounts}}' | tr ',' '\n' | sort | uniq -c`
  );
  const volumeUsage = new Map<string, number>();
  ...
  for (const line of result.stdout.split('\n')) {
    ...
    const sizeResult = await runCommand(
      `docker run --rm -v ${name}:/vol alpine du -sb /vol 2>/dev/null | cut -f1`
    );
...
export async function getSystemDF(): Promise<DockerSystemDF | null> {
...
export interface CleanupStats {
  ...
  buildCache: { bytes: number };
}

export async function cleanupDryRun(): Promise<CleanupStats> {
